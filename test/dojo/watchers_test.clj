...
(deftest test-watchers
  ...
  (defn rest-init [ctx opt]
    (atom 0))

  (defn rest-stat [ctx {st :state} ev]
    (swap! st inc))

  (def cfg
    {:wts {:rest/stat {:evs #{"w/resp"}
                       :init rest-init
                       :fn rest-stat}}
     :fxs {:notify notify}})

  (def ctx' (sut/start cfg))
  ctx'

  (def filename "logs.ndjson")

  (time
   (sut/report-from-file ctx' filename))
