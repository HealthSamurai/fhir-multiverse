...
(defn format-mw [h]
  (fn [req]
    (let [{{fmt :_format} :params :as req'} (prepare-request req)
          resp (h req')]
      (if-let [b (:body resp)]
        (cond
          (= "edn" fmt)
          (-> resp
              (assoc :body (zprint.core/zprint-str b))
              (assoc-in [:headers "content-type"] "text/edn"))
          (= "yaml" fmt)
          (-> resp
              (assoc :body (clj-yaml.core/generate-string b))
              (assoc-in [:headers "content-type"] "text/yaml"))
          :else
          (-> resp
